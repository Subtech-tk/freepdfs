        ...
        <div class="demo-blog__posts mdl-grid">
          <div class="mdl-card mdl-shadow--4dp mdl-cell mdl-cell--12-col">
            ...
            <?php
              ...
              if (isset($fname) && isset($lname) && isset($gender) && isset($dob) && isset($email) && isset($username) && isset($password) && isset($password_re)) 
              {
                if (!empty($fname) && !empty($lname) && !empty($gender) && !empty($dob) && !empty($email) && !empty($username) && !empty($password) && !empty($password_re)) 
                {
                  ...
                  $username=netutralize($username,$connection);
                  ...
                  if (read_db_entry($email,'emailid','userdetail')) 
                  {
                    $pass=false;
                    //echo "OOP's It seems the Email-id already is beign used by someone else." ;
                  }

                  // if the username already exist
                  if (read_db_entry($username,'username','userdetail')) 
                  {
                    $pass=false;
                    //echo "OOP's It seems the Username already is beign used by someone else." ;
                  }
                      ...
                      <legend>Email id :
                        <?php 
                          if (isset($email) && empty($email)) 
                            {
                              echo "<br/>Email is missing";
                            } 
                          if (isset($email) && !empty($email) && read_db_entry($email,'emailid','userdetail')){
                              echo "OOP's It seems the Email-id already is beign used by someone else." ;
                            }
                        ?>
                      </legend>
                      ...
                      <legend>Username :
                        <?php 
                          if (isset($username) && empty($username)) 
                            {
                              echo "<br/>Username is missing";
                            } 
                          if (isset($username) && !empty($username) && read_db_entry($username,'username','userdetail')) 
                            {
                              echo "OOP's It seems the Username already is beign used by someone else." ;
                            }
                        ?>
                      </legend>
                      <input class="mdl-textfield__input" id="username" type="text" name="username" value="<?php if (isset($username)) {echo "$username";}?>">
                    </fieldset>
                    <br/>
                    <fieldset>
                      <legend>Password :
