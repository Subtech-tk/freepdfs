...
//$uid=1;	//only for testing
	//echo "in comment_ins.php";
		...
		$result=$connection->query("SELECT `id`,`name`,`bref` FROM `books` WHERE `uploader`='$uid'");	//query to get book uploades by user from the main table i.e verified books
	...
	{		
		...
		?>
				<div id="featured">
					...
					<ul class="actions">
						<li><a href="<?php echo"bookembed.php?ref=$id"?>" class="button">Description</a></li>
					</ul>
				</div>
		<?php 
			}
